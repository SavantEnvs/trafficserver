...
# containable dependency. That closure is out of scope for this pass (heavy, hard to air-gap), so
# ENABLE_QUICHE stays OFF and fuzz_http3frame is simply not built. Upstream now adds the quic/
# subdirectory whenever TS_USE_QUIC OR TS_USE_QMUX, and quic builds without quiche, so ts::quic
# exists for the configure step without any stub of ours (see the cmake block below).
#
...
#      NO ts::quic STUB ANY MORE. We used to inject mayhem/cmake/mayhem-quic-stub.cmake via
#      -DCMAKE_PROJECT_INCLUDE to fabricate a dummy `ts::quic`, because tests/fuzzing/CMakeLists.txt
#      references that target unconditionally (for fuzz_http3frame, which we do not build) while
#      src/iocore/net/CMakeLists.txt added the real quic/ subdirectory only `if(TS_USE_QUIC)` —
#      false here, so nothing defined it. Upstream widened that to
#      `if(TS_USE_QUIC OR TS_USE_QMUX)`, so quic/ IS now added and defines ts::quic itself; our
#      stub, injected right after project(), got there first and configure died with
#      "add_library cannot create ALIAS target ts::quic because another target with the same name
#      already exists" (src/iocore/net/quic/CMakeLists.txt:33). The stub's own `if(NOT TARGET
#      ts::quic)` guard cannot help: at CMAKE_PROJECT_INCLUDE time nothing has defined it yet.
#      Do NOT re-add it. Upstream also made quic buildable WITHOUT quiche (`if(TS_HAS_QUICHE)` only
#      adds the quiche::quiche link), which is why dropping the stub pulls in no HTTP/3 toolchain.
#      ENABLE_QUICHE is still left at its default OFF — a plain, real-OpenSSL build.
