...
// Upstream added a 7th parameter, `uint32_t header_field_max_size`, to
// hpack_decode_header_block() (include/proxy/http2/HPACK.h). 32768 mirrors upstream's own
// HPACK test (src/proxy/http2/test_HPACK.cc: MAX_FIELD_SIZE) and Http2ConnectionState's
// default _header_field_max_size, so the harness bounds header fields as production does.
...
LLVMFuzzerTestOneInput(const uint8_t *input_data, size_t size_data)
{
  ...
  hpack_decode_header_block(indexing_table, headers.get(), input_data, size_data, MAX_REQUEST_HEADER_SIZE,
